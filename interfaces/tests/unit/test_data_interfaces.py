...
ENTITY_PERMISSIONS = [{'resource_name': 'cars', 'resource_type': 'TABLE', 'privileges': ['SELECT']}]
...
class DatabaseCharm(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.provider = ResourceProviderEventHandler(self, DATABASE_RELATION_NAME, RequirerCommonModel)
        self._servers_data = {}
...
class DataProvidesBaseTests(ABC):
    ...
    def test_relation_interface(self):
        ...
        self.harness.update_relation_data(self.rel_id, 'application', {self.DATABASE_FIELD: DATABASE})
...
class TestDatabaseProvides(DataProvidesBaseTests, unittest.TestCase):
    ...
    def test_on_resource_entity_requested_v0(self, _on_resource_entity_requested):
        ...
        assert [item.model_dump() for item in event.request.entity_permissions] == ENTITY_PERMISSIONS
        assert event.request.extra_user_roles == EXTRA_USER_ROLES
    ...
    def test_on_entity_permissions_changed_v0(self, _on_resource_entity_permissions_changed):
        ...
        assert event.request.resource == DATABASE
        assert event.request.entity_type == ENTITY_USER
        assert event.request.entity_permissions == TypeAdapter(list[EntityPermissionModel]).validate_python(
            ENTITY_PERMISSIONS
        )
        assert event.request.extra_user_roles == EXTRA_USER_ROLES
...
class TestDatabaseProvidesDynamicSecrets(ABC, unittest.TestCase):
    ...
    def test_peer_relation_interface(self, scope):
        """Check the functionality of each public interface function."""
        interface: OpsPeerRepositoryInterface = getattr(self.harness.charm, f'peer_relation_{scope}')
        relation_id = self.harness.charm.peer_relation.id
        ...
        secret = self.harness.charm.model.get_secret(label=f'{PEER_RELATION_NAME}.database.{scope}')
...
class ApplicationCharmDatabase(CharmBase):
    ...
    def __init__(self, *args):
        ...
        self.requirer = ResourceRequirerEventHandler(
            ...
            relation_name=DATABASE_RELATION_NAME,
            requests=[
                RequirerCommonModel(resource=DATABASE, extra_user_roles=EXTRA_USER_ROLES, salt='kkkkkkkk'),
        ...
        self.framework.observe(self.on[DATABASE_RELATION_NAME].relation_broken, self._on_relation_broken)
    ...
    def get_relation_size(log_message: str) -> int:
        num_of_relations = (
            re.search(r'relations: [0-9]*', log_message).group(0).replace('relations: ', '').strip()
        )
...
class TestDatabaseRequires(DataRequirerBaseTests, unittest.TestCase):
    ...
    def test_on_resource_created_secrets(self, _on_resource_created):
        ...
        assert self.harness.charm.requirer.is_resource_created(self.rel_id, event.response.request_id)
    ...
    def test_fetch_relation_data_secrets_fields(self):
        ...
        assert repository.get_secret_field('password', SecretGroup('user'), uri=secret.id) == 'test-password'
    ...
    def test_fetch_my_relation_data_and_fields_secrets(self):
        ...
        with self._caplog.at_level(logging.ERROR):
            ...
            assert 'This operation (get_field) can only be performed by the leader unit' in self._caplog.text
    ...
    def test_assign_relation_alias(self):
        ...
        assert self.harness.get_relation_data(self.rel_id, unit_name)['alias'] == CLUSTER_ALIASES[0]
        ...
        assert self.harness.get_relation_data(second_rel_id, unit_name)['alias'] == CLUSTER_ALIASES[1]
    ...
    def test_resource_events(self, is_leader: bool):
        ...
        for event in events:
            ...
            previous_event_diff = self.harness.get_relation_data(self.rel_id, f'{self.app_name}/0').get(
                'data', ''
            )
            ...
            assert all(isinstance(captured_event, event['event']) for captured_event in captured_events)
