...
from dpcharmlibs.interfaces.events import STATUS_FIELD
...
def harness(request: pytest.FixtureRequest) -> Harness:
    ...
    harness = Harness(_ProviderCharm, meta=metadata)
...
def requirer_harness(request: pytest.FixtureRequest) -> Harness:
    ...
    harness = Harness(_RequirerCharm, meta=metadata)
    harness.set_leader(True)
    harness.begin_with_initial_hooks()
    return harness
...
def _get_statuses_from_relation_data(
    harness: Harness, relation_id: int, provider_app: str | None = None
) -> list[RelationStatusDict]:
    app_name = provider_app if provider_app else harness.charm.app.name
    data = harness.get_relation_data(relation_id, app_name)
