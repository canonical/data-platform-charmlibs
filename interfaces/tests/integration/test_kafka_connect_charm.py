...
async def test_deploy_charms(ops_test: OpsTest, application_charm: Path, kafka_connect_charm: Path):
...
async def test_connect_client_relation_with_charm_libraries(
    ...
    requests = json.loads(
        await get_application_relation_data(ops_test, REQUIRER_APP_NAME, SOURCE_REL, 'requests') or '[]'
    )
    _request = requests[0]
