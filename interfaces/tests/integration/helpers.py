...
async def get_connection_info(
    ...
    path = await get_application_relation_data(ops_test, application_name, relation_name, 'path', relation_id)
...
async def get_application_relation_data(
    ...
    if relation_alias:
        ...
        relation_data = [
            ...
            if await get_alias_from_relation_data(ops_test, unit_name, next(iter(v['related-units'])))
            == relation_alias
        ]
    if related_endpoint:
        relation_data = [
            v for v in data[unit_name]['relation-info'] if v['related-endpoint'] == related_endpoint
        ]
...
async def get_secret_by_label(ops_test, label: str, owner: str = '') -> dict[str, str]:
    ...
    secret_ids = [secret_line.split()[0] for secret_line in secrets_raw[1].split('\n')[1:] if secret_line]
