...
async def test_deploy_charms(ops_test: OpsTest, application_charm, opensearch_charm):
    ...
    await asyncio.gather(
        ops_test.model.wait_for_idle(apps=[OPENSEARCH_APP_NAME], status='active', wait_for_exact_units=1),
        ops_test.model.wait_for_idle(apps=[APPLICATION_APP_NAME], status='active', wait_for_exact_units=1),
    )
...
async def test_opensearch_relation_with_charm_libraries_secrets(ops_test: OpsTest):
    ...
    await ops_test.model.add_relation(OPENSEARCH_APP_NAME, f'{APPLICATION_APP_NAME}:{INDEX_RELATION_NAME}')
    ...
    # Get the requests
    requests = json.loads(
        await get_application_relation_data(ops_test, APPLICATION_APP_NAME, INDEX_RELATION_NAME, 'requests')
        or '[]'
    )
    request = requests[0]
...
async def test_opensearch_roles_relation_with_charm_libraries_secrets(ops_test: OpsTest):
    """Test basic functionality of opensearch relation interface."""
    # Relate the charms and wait for them exchanging some connection data.
    await ops_test.model.add_relation(OPENSEARCH_APP_NAME, f'{APPLICATION_APP_NAME}:{ROLES_RELATION_NAME}')
    await ops_test.model.wait_for_idle(apps=APP_NAMES, status='active')
