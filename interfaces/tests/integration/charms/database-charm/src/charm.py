...
from ops.charm import ActionEvent, WorkloadEvent
...
class Charm(common.Charm):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        self.framework.observe(self.database.on.resource_entity_requested, self._on_resource_entity_requested)
        ...
        # Get/set/delete values on second-database relaton
        self.framework.observe(self.on.get_peer_relation_field_action, self._on_get_peer_relation_field)
        self.framework.observe(self.on.set_peer_relation_field_action, self._on_set_peer_relation_field)
        self.framework.observe(
        ...
        self.framework.observe(self.on.delete_peer_relation_field_action, self._on_delete_peer_relation_field)
    ...
    def _on_get_relation_field(self, event: ActionEvent):
        ...
        value = None
        model = self.database.interface.build_model(relation.id, DataContract, component=relation.app)
        for request in model.requests:
    ...
    def _on_get_other_peer_relation_field(self, event: ActionEvent):
        ...
        for unit, interface in self.peer_units_data_interfaces.items():
            ...
            value[unit.name.replace('/', '-')] = getattr(model, event.params['field'].replace('-', '_'))
