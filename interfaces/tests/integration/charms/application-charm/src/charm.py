...
class Charm(common.Charm):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        self.first_database = ResourceRequirerEventHandler(
            ...
            requests=[RequirerCommonModel(resource=database_name, extra_user_roles=EXTRA_USER_ROLES)],
        ...
        self.framework.observe(self.first_database.on.resource_created, self._on_first_database_created)
        ...
        self.first_database_username = ResourceRequirerEventHandler(
            ...
            requests=[
                RequirerCommonModel(resource=database_name, entity_type='USER', entity_name='testuser')
            ],
            response_model=ExtendedResponseModel,
        )
        self.framework.observe(self.first_database.on.resource_created, self._on_first_database_created)
        ...
        self.framework.observe(self.second_database.on.resource_created, self._on_second_database_created)
        self.framework.observe(
        ...
        self.framework.observe(self.database_clusters.on.resource_created, self._on_cluster_database_created)
        self.framework.observe(
        ...
        self.framework.observe(self.kafka_split_pattern.on.resource_created, self._on_kafka_topic_created)
        ...
        self.framework.observe(self.kafka.on.endpoints_changed, self._on_kafka_bootstrap_server_changed)
        ...
        self.framework.observe(self.connect_source.on.resource_created, self._on_connect_integration_created)
        ...
        self.opensearch = ResourceRequirerEventHandler(
            ...
            requests=[
                RequirerCommonModel(resource='test-index', extra_user_roles=EXTRA_USER_ROLES_OPENSEARCH)
            ],
            response_model=ResourceProviderModel,
        )
        self.framework.observe(self.opensearch.on.resource_created, self._on_opensearch_index_created)
    ...
    def _on_first_database_created(self, event: ResourceCreatedEvent) -> None:
        ...
        logger.info(f'first database credentials: {event.response.username} {event.response.password}')
    ...
    def _on_second_database_created(self, event: ResourceCreatedEvent) -> None:
        """Event triggered when a database was created for this application."""
        # Retrieve the credentials using the charm library.
        logger.info(f'second database credentials: {event.response.username} {event.response.password}')
    ...
    def _on_get_plugin_status(self, event: ActionEvent) -> None:
        ...
        plugin_status = 'enabled' if self.first_database.is_postgresql_plugin_enabled(plugin) else 'disabled'
    ...
    def _on_kafka_bootstrap_server_changed(self, event: ResourceEndpointsChangedEvent):
        ...
        logger.info(f'On kafka boostrap-server changed: bootstrap-server: {event.response.endpoints}')
    ...
    def _on_status_raised(self, event: StatusRaisedEvent):
        ...
        self.unit.status = ActiveStatus(f'Active Statuses: {[s.code for s in event.active_statuses]}')
    ...
    def _on_install(self, event: ops.InstallEvent) -> None:
        ...
        try:
            ...
            logger.exception(f'Failed to install etcdctl: {e}')
            event.defer()
            return
    ...
    def _on_put_etcd_action(self, event: ops.ActionEvent) -> None:
        ...
        for cert in certs:
            ...
            key = orig_key if orig_key.startswith('/') else f'/{cert.certificate.common_name}/{orig_key}'
    ...
    def _on_get_credentials_action_etcd(self, event: ops.ActionEvent) -> None:
        ...
        if not self.server_ca_chain:
            event.fail('The server CA chain is not available. Please wait for the server to provide it.')
            event.set_results({'ok': False})
            return
    ...
    def _on_get_certificates_action(self, event: ops.ActionEvent) -> None:
        ...
        certs_to_send = [cert.ca.raw if self.send_ca_option else cert.certificate.raw for cert in certs]
        event.set_results({
    ...
    def _install_etcdctl(self):
        ...
        tmp_dir = '/tmp/etcd_install'  # noqa: S108
        os.makedirs(tmp_dir, exist_ok=True)
        tar_path = os.path.join(tmp_dir, 'etcd.tar.gz')
        # Download tarball using urllib
        try:
            with urllib.request.urlopen(url) as response, open(tar_path, 'wb') as out_file:  # noqa: S310
