...
def check_apps_idle_period(status: jubilant.Status, *apps: str, idle_period: int) -> bool:
    return all(
        parse(unit.juju_status.since, ignoretz=True) + timedelta(seconds=idle_period) < datetime.now()
        for app in apps
        for unit in status.get_units(app).values()
    )


def get_cluster_endpoints(juju: Juju, app_name: str = ETCD_APP_NAME, tls_enabled: bool = False) -> str:
...
def get_certificate_from_unit(juju: Juju, unit: str, cert_type: TLSType, is_ca: bool = False) -> str | None:
