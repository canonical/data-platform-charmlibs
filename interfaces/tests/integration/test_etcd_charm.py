...
def test_deploy_charms(juju_lxd_model: Juju, application_charm: Path, install_etcdctl: None) -> None:
...
def test_relate_client_charm(juju_lxd_model: Juju):
    ...
    juju_lxd_model.wait(
        lambda status: apps_active_and_agents_idle(status, ETCD_APP_NAME, REQUIRER_APP_NAME, idle_period=10)
    )
...
def test_write_read_with_requirer(juju_lxd_model: Juju) -> None:
    ...
    with pytest.raises(TaskError) as task_error:
        juju_lxd_model.run(requirer_unit, 'put-etcd', params={'key': TEST_KEY, 'value': TEST_VALUE})
    ...
    key = 'test/foo'
    action = juju_lxd_model.run(requirer_unit, 'put-etcd', params={'key': key, 'value': TEST_VALUE})
