...
from .helpers import CHARMS_PATH, get_application_relation_data
...
async def test_raise_status(ops_test: OpsTest):
    ...
    assert '[4001]' in ops_test.model.applications[APPLICATION_APP_1].units[0].workload_status_message
    assert '[4002]' in ops_test.model.applications[APPLICATION_APP_2].units[0].workload_status_message
    ...
    assert '[1000, 4001]' in ops_test.model.applications[APPLICATION_APP_1].units[0].workload_status_message
    assert '[4002]' in ops_test.model.applications[APPLICATION_APP_2].units[0].workload_status_message

    status_schema_raw = json.load(open(f'{CHARMS_PATH}/database-charm/src/status-schema.json'))
    status_schema_map = {o.get('code'): o for o in status_schema_raw.get('statuses', [])}

    # Verify rel data matches status schema
    rel_data_1 = await get_application_relation_data(ops_test, APPLICATION_APP_1, RELATION_NAME, 'status')
    assert rel_data_1
...
async def test_resolve_status(ops_test: OpsTest):
    ...
    await ops_test.model.wait_for_idle(apps=APP_NAMES, status='active', idle_period=30)
    assert '[1000]' in ops_test.model.applications[APPLICATION_APP_1].units[0].workload_status_message
...
async def test_clear_statuses(ops_test: OpsTest):
    ...
    await ops_test.model.wait_for_idle(apps=APP_NAMES, status='active', idle_period=30)
    assert '[1000, 4002]' in ops_test.model.applications[APPLICATION_APP_1].units[0].workload_status_message
