...
async def test_deploy_charms(ops_test: OpsTest, backward_compatibility_charm: Path, database_charm: Path):
    ...
    await ops_test.model.wait_for_idle(apps=[DATABASE_APP_NAME], status='active', wait_for_exact_units=1)
    await ops_test.model.wait_for_idle(apps=[APPLICATION_APP_NAME], status='active', wait_for_exact_units=1)
...
async def test_backward_relation_with_charm_libraries_secrets(ops_test: OpsTest):
    ...
    rel = await ops_test.model.add_relation(DATABASE_APP_NAME, f'{APPLICATION_APP_NAME}:{RELATION_NAME}')
    ...
    database = await get_application_relation_data(ops_test, APPLICATION_APP_NAME, RELATION_NAME, 'database')
...
async def test_backward_relation_with_prefix_and_username(ops_test: OpsTest):
    ...
    # Relate the charms and wait for them exchanging some connection data.
    await ops_test.model.add_relation(DATABASE_APP_NAME, f'{APPLICATION_APP_NAME}:{prefix_relation}')
    await ops_test.model.wait_for_idle(apps=APP_NAMES, status='active')
