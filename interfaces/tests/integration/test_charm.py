...
DATABASE_APP_METADATA = yaml.safe_load(Path(f'{CHARMS_PATH}/database-charm/metadata.yaml').read_text())
DATABASE_DUMMY_APP_METADATA = yaml.safe_load(
    Path(f'{CHARMS_PATH}/dummy-database-charm/metadata.yaml').read_text()
)
...
async def test_deploy_charms(
    ...
    await asyncio.gather(
        ...
        ops_test.model.deploy(
            database_charm,
            resources={
                'database-image': DATABASE_APP_METADATA['resources']['database-image']['upstream-source']
            },
    ...
    await asyncio.gather(
        ...
        ops_test.model.wait_for_idle(apps=[DATABASE_APP_NAME], status='active', wait_for_exact_units=NUM_DB),
        ops_test.model.wait_for_idle(
...
async def test_peer_relation_set_secret(component, ops_test: OpsTest):
    ...
    secret = await get_secret_by_label(ops_test, f'database-peers.dummy-database.{component}', owner)
    ...
    secret = await get_secret_by_label(ops_test, f'database-peers.dummy-database.{component}.mygroup', owner)
...
async def test_other_peer_relation(ops_test: OpsTest):
    ...
    for main_unit in units:
        action = await main_unit.run_action('get-other-peer-relation-field', **{'field': 'monitor-password'})
        await action.wait()
        ...
        action = await main_unit.run_action('get-other-peer-relation-field', **{'field': 'non-secret-field'})
        await action.wait()
...
async def test_other_peer_relation_scale(ops_test: OpsTest):
    ...
    await ops_test.model.wait_for_idle(apps=[DATABASE_APP_NAME], status='active', wait_for_exact_units=2)
    units = ops_test.model.applications[DATABASE_APP_NAME].units
    ...
    await ops_test.model.wait_for_idle(apps=[DATABASE_APP_NAME], status='active', wait_for_exact_units=3)
...
async def test_postgresql_plugin(ops_test: OpsTest):
    ...
    action = await ops_test.model.units.get(unit_name).run_action('get-plugin-status', **{'plugin': 'citext'})
    await action.wait()
...
async def test_tls_integration_after_initial_integration(ops_test: OpsTest):
    ...
    await ops_test.model.wait_for_idle(apps=[APPLICATION_APP_NAME, DATABASE_APP_NAME], status='active')
...
async def test_two_applications_dont_share_the_same_relation_data(ops_test: OpsTest, application_charm):
...
async def test_an_application_can_connect_to_multiple_database_clusters(ops_test: OpsTest, database_charm):
