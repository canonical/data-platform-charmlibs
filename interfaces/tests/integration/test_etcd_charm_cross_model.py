...
def test_deploy_charms(
    juju_lxd_model: Juju, requirer_model: Juju, application_charm: Path, install_etcdctl: None
) -> None:
    ...
    # The application charm is deployed into a separate model for cross-model relations
    # set data in the relation application databag using only the leader unit).
    requirer_model.deploy(application_charm, app=REQUIRER_APP_NAME, num_units=1)
    ...
    requirer_model.wait(
        lambda status: apps_active_and_agents_idle(status, REQUIRER_APP_NAME, idle_period=10),
        timeout=1200,
        successes=1,
    )


@pytest.mark.log_errors_allowed('etcdctl user command failed')
def test_relate_client_charm(juju_lxd_model: Juju, requirer_model: Juju):
    ...
    secret = get_secret_by_label_jubilant(juju_lxd_model, label=f'{PEER_RELATION}.{ETCD_APP_NAME}.app')
    ...
    for common_name in common_names:
        user_roles = get_user(endpoints, common_name, user=INTERNAL_USER, password=password, tls_enabled=True)
    ...
    for unit_name in juju_lxd_model.status().get_units(ETCD_APP_NAME):
        client_cas = get_certificate_from_unit(juju_lxd_model, unit_name, TLSType.CLIENT, is_ca=True)
...
def test_write_read_with_requirer(requirer_model: Juju) -> None:
    """Test write and read to the key prefix with the requirer charm."""
    requirer_unit = next(iter(requirer_model.status().get_units(REQUIRER_APP_NAME)))

    # write to the key prefix
    with pytest.raises(TaskError) as task_error:
        requirer_model.run(requirer_unit, 'put-etcd', params={'key': TEST_KEY, 'value': TEST_VALUE})
    ...
    key = 'test/foo'
    action = requirer_model.run(requirer_unit, 'put-etcd', params={'key': key, 'value': TEST_VALUE})
    assert action.status == 'completed', 'Action should succeed'

    # read from the key prefix
    action = requirer_model.run(requirer_unit, 'get-etcd', params={'key': key})
    assert action.status == 'completed', 'Action should succeed'
    common_names = get_requirer_common_names(requirer_model)
...
@pytest.mark.log_errors_allowed('etcdctl put failed')
def test_update_mtls_cert(juju_lxd_model: Juju, requirer_model: Juju):
